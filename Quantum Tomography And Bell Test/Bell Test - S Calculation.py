...
DATA_DIR = r"/Users/noamartstain/Documents/PythonCourse/Physics Lab B/Quantum-Tomography-And-Bell-Test--main/"

# --- GLOBAL THRESHOLDS ---
# Set these values before running the script!
ALICE_THRESHOLD = 45000 #30000 # Replace with your chosen threshold for Alice
BOB_THRESHOLD = 12000 #18500.00  # Replace with your chosen threshold for Bob
DEAD_TIME_FRAMES = 20
...
for file_name in FILES:
    ...
    if SHOW_VALIDATION_PLOTS:
        ...
        fig.suptitle(f"Pulse Validation: {base_name}\n(Close window to continue)", fontsize=14, fontweight='bold')
        ...
        # Plot at the intersection (y = ALICE_THRESHOLD) instead of the peak height
        ...
        # Plot at the intersection (y = BOB_THRESHOLD) instead of the peak height
        ...
        # Create a folder specifically for these plots to keep your directory clean
        plot_dir = os.path.join(DATA_DIR, "Validation_Plots")
        os.makedirs(plot_dir, exist_ok=True)

        # Generate a clean filename based on the .npz file's name
        # e.g., "results_Bell_0__22.5.npz" -> "Validation_Bell_0__22.5.png"
        plot_filename = base_name.replace("results_", "Validation_").replace(".npz", ".png")
        ...
        plt.show(block=True)
        # Save thresholded data for the next phase
...
# IMPORTANT: Set this to the total length of your time_axis array
# If your files have slightly different lengths, use the average or maximum
TOTAL_FRAMES = 40 # Replace with actual len(time_axis) from your data!
...
def get_E_with_propagated_error(alpha, beta, res_dict, m_total):
    ...
    # 1. Get individual N counts
    N1 = res_dict.get((a, b), 0)                # N_ab
    N2 = res_dict.get((a_perp, b_perp), 0)      # N_a_perp_b_perp
    N3 = res_dict.get((a_perp, b), 0)           # N_a_perp_b
    N4 = res_dict.get((a, b_perp), 0)           # N_a_b_perp

    # 2. Calculate Binomial Variance for each N
    # sigma_N^2 = N * (1 - N / M_total)
    ...
    # 4. Propagate error to E using partial derivatives
...
# Calculate the four expectation values and their propagated errors
...
# Propagate the error for S (Addition in quadrature)
