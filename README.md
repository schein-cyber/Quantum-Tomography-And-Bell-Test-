...
This repository contains the data analysis scripts and numerical simulations for our experiment, which utilizes a classical pulsed laser system to empirically emulate quantum state tomography and the violation of the CHSH Bell inequality. For the tomography portion, the code processes raw measurement videos of randomly polarized pulses—split between two detection stations—to reconstruct experimental density matrices for sequences of 10, 25, and 50 pulses across varying polarization bases. To rigorously validate this data pipeline, a corresponding simulation suite models the exact optical setup for extended sequences of up to 1000 bits and plots the resulting theoretical density matrices. In the second phase, the scripts evaluate the system's capacity to emulate quantum entanglement by analyzing empirical coincidence events from 16 unique linear polarizer angle configurations. Ultimately, the code computes a Bell parameter strictly greater than 2, which violates the CHSH inequality and refutes the classical local hidden-variable theory.
