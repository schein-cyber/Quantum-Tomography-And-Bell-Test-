# Divide And Conquer: Emulating Quantum Tomography and Bell's Test with Classical Light


This repository contains the data analysis scripts and numerical simulations for our experiment, which utilizes a classical pulsed laser system to empirically emulate quantum state tomography and the violation of the CHSH Bell inequality. For the tomography portion, the code processes raw measurement videos of randomly polarized pulses—split between four detectors to reconstruct experimental density matrices for sequences of 10, 25, and 50 pulses. To rigorously validate this data pipeline, a corresponding simulation models the exact optical setup for extended sequences of 50, 100 and 1000 bits, and plots the resulting density matrices. In the second phase, the scripts evaluate the system's capacity, now with only two detectors, to violate the Bell inequality by analyzing empirical coincidence events from 16 unique linear polarizer angle configurations. Ultimately, the code computes a Bell parameter strictly greater than 2, which violates the CHSH inequality and refutes the classical local hidden-variable theory.
...
### Description 
This code contains the simulation of the quantum tomography experiment. The system is completely mimicked by the code, including the optical setup and the random polarization of the laser. The code evaluates 3 total bit amounts, 50, 100 and 1000. It then constructs the density matrices of the corresponding measurements, and plots them on a 3d graph, similarly to the density matrices codes. Additionally, the code mimics the HWP by switching Bob's H and V states, plotting the corresponding density matrices as well.
...
### inputs 
The 3 raw intesity npz files produced from the previous code, named "results_tomography_i_bits.npz" for i in [10, 25, 50].
...
### inputs 
The 3 thresholded intensity npz files  produced from the previous code, named "thresholded_results_tomography_i_bits.npz" for i in [10, 25, 50]. 
...
### Description 
This code is to be used after code 6 has been implemented. It takes the raw intensity data files of the bell test measurements, allows the user to choose a uniform threshold across all 16 videos for Alice and Bob separetly. It then measurse the overlap between Alice's and Bob's measurements for each video, by scanning the binarized graphs across 5 frames to both sides and counts these overlaps to calculate the Bell parameter S defined in our article. It then gives this S value as an output. To estimate the error, each coincidence count is treated as a binomial variable, since each pulse has a probability of being a coincidence, and is independent of the others. n = N_pulses, and we estimated p = N(a,b)/N_pulses, and then used the standard deviation of a binomial variable as the error for the coincidence count. From the error in each count, the code estimates the error dS and outputs it as well. It additionally plots 16 conincidence graphs, 1 for each angle configuration. Finally, it saves 16 thresholded intensity graphs of both Alice and Bob, for each corresponding angle, with the threshold present as a line. These files are save as .png files, in a newly generated folder.
